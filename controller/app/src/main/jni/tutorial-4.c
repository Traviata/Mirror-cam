...
static void gst_native_init (JNIEnv* env, jobject thiz, jstring addr, jstring strport) {
    ...
    //pipeline=g_strjoin ("","udpsrc port=8554 ! application/x-rtp ! rtph264depay ! h264parse tee name=t \
                           t. ! queue ! avdec_h264 ! autovideosink sync=false \
                           t. ! queue ! mp4mux fragment-duration=10 ! filesink location=/sdcard/rec.mp4",NULL );
    pipeline="udpsrc port=8554 ! application/x-rtp !  rtph264depay ! h264parse ! avdec_h264 ! videoflip method=none ! autovideosink sync=false";
...
static void gst_native_snapshot(JNIEnv* env, jobject thiz, jstring addr, jstring strport){
        CustomData *data =pic;
        SET_CUSTOM_DATA (env, thiz, custom_data_field_id, data);
        ...
        pipeline="udpsrc port= 8554 num-buffers=250 ! application/x-rtp !  rtph264depay ! h264parse ! decodebin ! jpegenc ! filesink location=/sdcard/screenshot.jpg";
...
static JNINativeMethod native_methods[] = {
        ...
        { "nativeSnapshot","(Ljava/lang/String;Ljava/lang/String;)V", (void *) gst_native_snapshot},
...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    jclass klass = (*env)->FindClass (env, "com/test/controller/MainActivity");
