...
static gboolean refresh_ui (CustomData *data) {
    ...
    if (!data || !data->pipeline1 || data->state < GST_STATE_PAUSED)
        return TRUE;

    /* If we didn't know it yet, query the stream duration */
    if (!GST_CLOCK_TIME_IS_VALID (data->duration)) {
        if (!gst_element_query_duration (data->pipeline1, GST_FORMAT_TIME, &data->duration)) {
            GST_WARNING ("Could not query current duration");
        }
    }

    if (!gst_element_query_position (data->pipeline1, GST_FORMAT_TIME, &position)) {
...
static void execute_seek (gint64 desired_position, CustomData *data) {
    ...
    if (GST_CLOCK_TIME_IS_VALID (data->last_seek_time) && diff < SEEK_MIN_DELAY) {
        ...
        gst_element_seek_simple (data->pipeline1, GST_FORMAT_TIME, GST_SEEK_FLAG_FLUSH | GST_SEEK_FLAG_KEY_UNIT, desired_position);
...
static void eos_cb (GstBus *bus, GstMessage *msg, CustomData *data) {
    ...
    data->is_live = (gst_element_set_state (data->pipeline1, GST_STATE_PAUSED) == GST_STATE_CHANGE_NO_PREROLL);
    execute_seek (0, data);
}
...
static void check_media_size (CustomData *data) {
    ...
    g_object_get (data->pipeline1, "video-sink", &data->video_sink, NULL);
...
static void check_initialization_complete (CustomData *data) {
    ...
    if (!data->initialized && data->native_window && data->main_loop) {
        ...
        //gst_video_overlay_set_window_handle (GST_VIDEO_OVERLAY (data->pipeline1), (guintptr)data->native_window);
...
static void *app_function (void *userdata) {
    ...
    /* Build pipeline */
    //data->pipeline1 = gst_parse_launch("playbin", &error);
    //data->pipeline1 = gst_parse_launch("appsrc ! videoconvert  ! video/x-raw,  width=640, heigh=480 ! videoconvert ! x264enc tune=zerolatency ! rtph264pay ! udpsink host=192.168.0.100 port=5000", &error);  //sending audio success
    //data->pipeline1 = gst_parse_launch("udpsrc port=5001 caps=\"application/x-rtp\" ! queue ! rtppcmudepay ! mulawdec ! audioconvert ! autoaudiosink sync=false", &error); //receiving audio success
    //data->pipeline1 = gst_parse_launch("openslessrc  ! audioconvert ! audio/x-raw, channels=1, rate=16000  ! rtpL16pay ! udpsink host=192.168.0.100 port=5001", &error);  //sending audio success
    //data->pipeline1 = gst_parse_launch("tcpclientsrc host=192.168.0.100 port=5001 ! audio/x-raw, format=(string)S16LE, channels=(int)2, rate=(int)44100 ! autoaudiosink sync=false", &error);
    //data->pipeline1 = gst_parse_launch("tcpclientsrc host=116.37.63.195 port=8555 ! audio/x-raw, format=(string)S16LE, channels=(int)2, rate=(int)44100 ! autoaudiosink sync=false", &error);


    data->pipeline1 = gst_parse_launch(pipeline, &error);
    ...
    /* Disable subtitles */
    g_object_get (data->pipeline1, "flags", &flags, NULL);
    flags &= ~GST_PLAY_FLAG_TEXT;
    g_object_set (data->pipeline1, "flags", flags, NULL);

    /* Set the pipeline to READY, so it can already accept a window handle, if we have one */
    data->target_state = GST_STATE_READY;
    gst_element_set_state(data->pipeline1, GST_STATE_READY);

    data->aug_pipeline=gst_parse_launch("autovideosink sync=false", &error);
    //data->video_sink = gst_bin_get_by_interface(GST_BIN(data->aug_pipeline), GST_TYPE_VIDEO_OVERLAY);
    data->video_sink = gst_bin_get_by_interface(GST_BIN(data->pipeline1), GST_TYPE_VIDEO_OVERLAY);
...
static void gst_native_init (JNIEnv* env, jobject thiz, jstring addr, jstring strport, jstring camdir) {
    ...
    const gchar *char_uri1 = (gchar*)(*env)->GetStringUTFChars (env, addr, NULL);       //주소, 포트번호, cam앞뒤를 입력 받아 전달
    const gchar *char_uri2 = (gchar*)(*env)->GetStringUTFChars (env, strport, NULL);
    const gchar *char_uri3 = (gchar*)(*env)->GetStringUTFChars (env, camdir, NULL);

/*
  pipeline=g_strjoin ("","ahcsrc device=", char_uri3 ,"  ! tee name=t \
  		  t. ! queue ! videoflip method=clockwise ! autovideosink sync=false \
  		  t. ! queue ! video/x-raw,  width=320, heigh=500 ! videoconvert ! x264enc tune=zerolatency !  rtph264pay ! udpsink host=", \
  		  char_uri1, " port=" , char_uri2, NULL);
...
*/
    ...
    //pipeline=g_strjoin ("","ahcsrc device=", char_uri3 ,"  ! tee name=t \
            ...
            //   ahcsrc device=0 이면 후면 카메라, device=1이면 전면 카메라가 선택
    pipeline=g_strjoin ("","ahcsrc device=", char_uri3 ," ! video/x-raw,  width=320, heigh=240 !videoconvert ! x264enc tune=zerolatency ! rtph264pay ! udpsink host=", \
    		  char_uri1, " port=" , char_uri2, NULL);
    ...
    (*env)->ReleaseStringUTFChars (env, camdir, char_uri3);
...
static void gst_native_picture(JNIEnv* env, jobject thiz) {
    CustomData *data = g_new0 (CustomData, 1);
    ...
    pipeline = "udpsrc num-buffers=250 ! application/x-rtp ! rtph264depay! h264parse ! decodebin ! jpegenc ! filesink location = /sdcard/DCIM/test.jpg" ;
...
void gst_native_set_uri (JNIEnv* env, jobject thiz, jstring uri) {
    CustomData *data = GET_CUSTOM_DATA (env, thiz, custom_data_field_id);
    if (!data || !data->pipeline1) return;
    const jbyte *char_uri = (*env)->GetStringUTFChars (env, uri, NULL);
    GST_DEBUG ("Setting URI to %s", char_uri);
    if (data->target_state >= GST_STATE_READY)
        gst_element_set_state (data->pipeline1, GST_STATE_READY);
    g_object_set(data->pipeline1, "uri", char_uri, NULL);
    (*env)->ReleaseStringUTFChars (env, uri, char_uri);
    data->duration = GST_CLOCK_TIME_NONE;
    data->is_live = (gst_element_set_state (data->pipeline1, data->target_state) == GST_STATE_CHANGE_NO_PREROLL);
}
...
static void gst_native_play (JNIEnv* env, jobject thiz) {
    ...
    data->is_live = (gst_element_set_state (data->pipeline1, GST_STATE_PLAYING) == GST_STATE_CHANGE_NO_PREROLL);
}
...
static void gst_native_surface_init (JNIEnv *env, jobject thiz, jobject surface) {
    ...
    if (data->native_window) {
        ...
        if (data->native_window == new_native_window) {
            ...
            if (data->video_sink) {
                //gst_video_overlay_expose(GST_VIDEO_OVERLAY (data->pipeline1));
                //gst_video_overlay_expose(GST_VIDEO_OVERLAY (data->pipeline1));
...
static void gst_native_surface_finalize (JNIEnv *env, jobject thiz) {
    ...
    if (data->video_sink) {
        //gst_video_overlay_set_window_handle (GST_VIDEO_OVERLAY (data->pipeline1), (guintptr)NULL);
        //gst_video_overlay_prepare_window_handle (GST_VIDEO_OVERLAY (data->video_sink));
...
static JNINativeMethod native_methods[] = {
        { "nativepicture", "()V", (void *) gst_native_picture },
...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    jclass klass = (*env)->FindClass (env, "com/test/camera/Tutorial4");
